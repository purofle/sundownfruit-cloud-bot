...
fn schema() -> UpdateHandler<anyhow::Error> {
    ...
    dialogue::enter::<Update, InMemStorage<BotState>, BotState, _>().branch(handler)
}

async fn start(bot: Bot, _: MyDialogue, msg: Message) -> Result<()> {
    bot.send_message(msg.chat.id, "Welcome to use Sundownfruit Cloud!")
        .await?;
    Ok(())
}
...
async fn create(bot: Bot, dialogue: MyDialogue, msg: Message) -> Result<()> {
    bot.send_message(msg.chat.id, "Initiating VM creation...")
        .await?;
